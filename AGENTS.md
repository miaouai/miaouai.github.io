...
1. **开始前**: 先读 `SESSION-STATE.md`，看看是否有未完成的任务需要继续
2. **开始时**: 更新 `SESSION-STATE.md`，写下任务名称、步骤清单、开始时间
3. **每完成一步**: 立即更新该步骤状态，记录关键信息和下一步
4. **如果感觉要断**: 赶紧把当前进度写进文件，包括做到哪了、下次从哪接
5. **完成后**: 标记任务为"已完成"并清空或归档

**为什么要这样做？**
- 会话可能随时断开
- token 限制可能导致上下文被截断
- 主动保存比被动恢复靠谱得多
- 即使忘了，SESSION-STATE.md 也会告诉你该干嘛

**核心原则：重要的进度信息不保存在"脑子"里，只保存在文件里！**
